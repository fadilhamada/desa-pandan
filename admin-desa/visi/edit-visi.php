<?php
    ...
    $data = jabatan("SELECT * FROM visi WHERE id = $id")[0];
    ...
    if(isset($_POST['submit'])) {
        ...
        if(!$deskripsi || $deskripsi == "") {
            echo "<script>
                    ...
                    document.location.href = 'post-visi.php';
                </script>";
            return false;
        }
        ...
        $query = "UPDATE visi SET deskripsi = ? WHERE id = ?";
    ...
    <div class="w-full pt-16" id="margin">
        <div class="px-5 py-2">
            <div class="font-bold py-3 text-base flex gap-3 mb-3">
                ...
                <p>/</p>
                <a href="/web-desa/admin-desa/visi.php">Visi desa</a>
                <p>/</p>
                <h1 class="text-[#06D001]">Tambah data</h1>
            </div>
            <form action="" method="POST">
                <div class="mb-5">
                    <textarea name="deskripsi" id="deskripsi" class="border w-full height rounded-md px-3 py-2" require><?= $data['deskripsi']; ?></textarea>
                </div>
                <div class="flex justify-end gap-4">
                    <button type="submit" name="submit" class="bg-blue-500 text-white px-3 py-2 rounded-md shadow-md">Simpan</button>
                    <a href="../visi.php" class="bg-red-500 text-white px-3 py-2 rounded-md shadow-md">Kembali</a>
                </div>
            </form>
        </div>
    </div>
